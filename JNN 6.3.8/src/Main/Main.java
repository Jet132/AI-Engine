...
class run implements Runnable {
	...
	public void run() {
		for (int i = 0; i < 500; i++) {
			...
			JNN.setInput(0, Main.player[AI][0] - Main.food[0], AI);
			JNN.setInput(1, Main.player[AI][1] - Main.food[1], AI);
			JNN.setInput(2, Main.checkPosition(Main.player[AI][0] - 1, Main.player[AI][1]), AI);
			JNN.setInput(3, Main.checkPosition(Main.player[AI][0] + 1, Main.player[AI][1]), AI);
			JNN.setInput(4, Main.checkPosition(Main.player[AI][0], Main.player[AI][1] - 1), AI);
			JNN.setInput(5, Main.checkPosition(Main.player[AI][0], Main.player[AI][1] + 1), AI);
			JNN.setInput(6, Main.checkPosition(Main.player[AI][0] - 1, Main.player[AI][1] - 1), AI);
			JNN.setInput(7, Main.checkPosition(Main.player[AI][0] - 1, Main.player[AI][1] + 1), AI);
			JNN.setInput(8, Main.checkPosition(Main.player[AI][0] + 1, Main.player[AI][1] - 1), AI);
			JNN.setInput(9, Main.checkPosition(Main.player[AI][0] + 1, Main.player[AI][1] + 1), AI);

			JNN.runAgent(AI);

			if (JNN.getOutput(0, AI) < 0.5) {
				if (Main.player[AI][0] > 0) {
					Main.player[AI][0]--;
				}
			} else if (JNN.getOutput(0, AI) > 0.5) {
				if (Main.player[AI][0] < Main.gridSize) {
					Main.player[AI][0]++;
				}
			}

			if (JNN.getOutput(1, AI) < 0.5) {
				if (Main.player[AI][1] > 0) {
					Main.player[AI][1]--;
				}
			} else if (JNN.getOutput(1, AI) > 0.5) {
