...
public final class Label extends JLabel {
	...
	protected void paintComponent(Graphics g) {
		...
		g.fillRect(
				(blocksize)
						* Main.player[Main.AIEngine.getAgentIDPerFitnessList(0)][0],
				(blocksize)
						* Main.player[Main.AIEngine.getAgentIDPerFitnessList(0)][1],
				blocksize, blocksize);
		...
		for (int i = 0; i < 20; i++) {
			g.drawString(String.valueOf(Main.AIEngine.getAgentIDPerFitnessList(i)) + ": "
					+ String.valueOf(Main.AIEngine.getAgentFitnessPerFitnessList(i)), 100, 20 + i * 10);
		}
